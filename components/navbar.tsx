...
export function Navbar({ onMenuClick }: { onMenuClick: () => void }) {
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-[260px] h-16 bg-white border-b border-outline-variant flex items-center justify-between px-4 lg:px-8 z-40">
      <div className="flex items-center gap-4 flex-1 max-w-xl">
        <button 
          onClick={onMenuClick}
          className="p-2 -ml-2 text-on-surface-variant hover:bg-surface-container rounded-md lg:hidden"
        >
          <Menu size={20} />
        </button>
